...
class VerificationService:
    ...
    def validate_phone_number(self, phone_number: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'country_code': '',
            'error': (
                'Invalid phone number format. Please use a valid Zambia number '
                '(e.g., 0971234567) or international format (e.g., +260971234567).'
            )
        }
    ...
    def send_phone_otp(self, user) -> Dict[str, Any]:
        ...
        user.save(update_fields=[
            'phone_otp', 'phone_otp_created_at', 'phone_attempts'
        ])
        ...
        try:
            ...
            logger.info(
                f"OTP for {normalized_phone} sent via email to {user.email}"
            )
            ...
            return {
                'status': 'otp_sent',
                'message': (
                    f'OTP generated for {normalized_phone}. '
                    f'(Check console for OTP)'
                ),
                'phone_number': normalized_phone,
                'via': 'console',
                'expires_in': 10,
    ...
    # ============================================
    # PHASE 2: EMAIL VERIFICATION (independent from phone)
    # ============================================
    ...
    def submit_verification(self, user, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Verification submitted for user {user.email} (ID: {verification.id})"
        )
    ...
    def approve_verification(
        ...
        return {
            'verification': verification,
            'message': (
                f"Verification for {user.full_name} has been approved. "
    ...
    def reject_verification(
        self, admin, verification_id: int, reason: str, notes: str = ''
    ) -> Dict[str, Any]:
    ...
    def update_phone_number(self, user, new_phone_number: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Phone number updated for user {user.email}: "
            f"{old_phone} -> {normalized_phone}"
        )
    ...
    def update_email(self, user, new_email: str) -> Dict[str, Any]:
        """
        ...
        - Preserves phone_verified and identity verification
        """
        ...
        logger.info(
            f"Email updated for user {user.id}: {old_email} -> {new_email}"
        )

        send_result = self.send_email_verification(user)

        return {
            'status': 'updated',
            'message': (
                'Email updated. A verification email has been sent to the new address.'
            ),
            'email': new_email,
            'email_sent': send_result.get('status') == 'email_sent',
            'next_step': 'email_verification',
        }
    ...
    def get_verification_history(self, user) -> List[Dict[str, Any]]:
        ...
        Returns a list of past verification attempts with status,
        submission date, review date, and document count.
        """
        ...
        for v in verifications:
            doc_count = VerificationDocument.objects.filter(
                verification=v
            ).count()
            ...
            history.append({
                ...
                'status_display': v.get_verification_status_display(),
    ...
    def _get_status_message(self, verification: IdentityVerification) -> str:
        ...
        return status_messages.get(
            verification.verification_status, 'Unknown status.'
        )
