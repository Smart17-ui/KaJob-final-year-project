...
from apps.common.constants import AssignmentStatus   # 🆕 needed for CHOICES
    ...
    def _get_summary(self, applications, assignments, reviews):
        ...
        return {
            ...
            'total_earnings': (
                ...
                .aggregate(Sum('job__budget'))['job__budget__sum'] or 0
            ),
            'average_rating': reviews.aggregate(Avg('rating'))['rating__avg'] or 0,
            'total_reviews': reviews.count(),
            'completion_rate': (
    ...
    def get(self, request):
        ...
        for assignment in assignments:
            review = Review.objects.filter(
                job=assignment.job, reviewee=worker
            ).first()
            ...
            result.append({
                ...
                'category': (
                    assignment.job.category.name
                    if assignment.job.category else None
                ),
                'client_name': assignment.job.client.full_name,
                'budget': assignment.job.budget,
                'status': assignment.status,
